...
int main() {
 ...
 if ( (buf_id = shm_open(shar_object, O_CREAT|O_RDWR, 0666)) == -1 ) {
  ...
  while (1) {
    ...
    while(buffer->store[cur_id] != -1) {
            printf("    --> checking store: store[%d] = %d\n", cur_id, buffer->store[cur_id]);
            cur_id = (cur_id + 1) % BUF_SIZE;
    }
    // Запись в первую свободную ячейку
    buffer->store[cur_id] = rand() % 11; // число от 0 до 10
    ...
    printf("Producer %d writes value = %d to cell [%d]\n",
           pid, buffer->store[cur_id], cur_id);
